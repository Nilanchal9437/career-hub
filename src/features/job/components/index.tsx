...
function getRepeatIntervalCode(days: number) {
  const seconds = days * 24 * 60 * 60; // Convertir les jours en secondes
  return `r${seconds}`;
}
interface JobPost {
  title: string;
...
export default function Jobs() {
  const [jobTitle, setJobTitle] = React.useState("Développeur logiciel");
  const [location, setLocation] = React.useState("États-Unis");
      ...
      <section className="pt-8 md:pt-12">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="font-extrabold text-2xl md:text-3xl text-gray-900 flex items-center gap-2">
            Découvrez les dernières offres d’emploi en IT & Logiciel
          </h1>
        </div>
        <p className="text-gray-500 mb-6 md:mb-8 text-sm md:text-base">
          Offres d’emploi réelles pour développeurs et professionnels tech —
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex flex-col md:flex-row gap-4 md:gap-6 mb-8">
          <div className="flex-1 min-w-[150px]">
            ...
            <input
              ...
              placeholder="Titre de poste ou mot-clé"
          ...
          <div className="flex-1 min-w-[150px]">
            ...
            >
              <option value={getRepeatIntervalCode(1)}>
                Dernières 24 heures
              </option>
              <option value={getRepeatIntervalCode(7)}>Dernière semaine</option>
              <option value={getRepeatIntervalCode(30)}>Dernier mois</option>
            </select>
          </div>
          <div className="flex-1 min-w-[150px]">
            <label className="block text-xs font-semibold text-gray-700 mb-1">
